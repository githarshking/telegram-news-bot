...
async def myplan_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """/myplan — Check all subscription statuses."""
    user_id = update.effective_user.id
    
    # We call the new function from db.py
    ...
    # Build a message string piece by piece
    response_lines = ["📋 *Your Subscriptions*\n"]
    
    for plan_data in user_plans:
        expiry = plan_data["expiry_date"]
        days_left = (expiry - date.today()).days
        
        plan_info = PLANS.get(plan_data["plan"], {})
        plan_display = plan_info.get("name", plan_data["plan"])

        if days_left < 0:
            response_lines.append(f"❌ *{plan_display}*: Expired on {expiry}")
        else:
            response_lines.append(f"✅ *{plan_display}*: *{days_left}* days left (Expires: {expiry})")
...
def run_dummy_server():
    ...
    # hugging face uses the PORT environment variable, defaulting to 7860
def run_dummy_server():
    """Run FastAPI on port 7860 to satisfy Hugging Face health checks."""
    uvicorn.run(app_api, host="0.0.0.0", port=7860, log_level="warning")
